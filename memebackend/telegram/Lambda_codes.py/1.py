...
def lambda_handler(event, context):
    try:
        if "source" in event and event["source"] == "aws.events":
            ...
            "httpMethod" in event
            and event["headers"].get("X-React-App-Request") == "true"
        ):
...
def handle_cloudwatch_event(
    ...
    if int(current_time.strftime("%H")) <= 16:
        ...
        item = table.get_item(Key={"id": next_round_hour_time})
        item = item.get("Item")
    ...
    if int(current_time.strftime("%H")) >= 9:
        ...
        # delete the slot:
        table.delete_item(Key={"id": last_round_hour_time})
        # delete the related chat (if exists):
        response = table.scan(
            FilterExpression=Attr("appointment_id").exists()
            & Attr("appointment_id").eq(last_round_hour_time)
        )
    ...
    if (
        current_time.strftime("%A") != "Thursday"
        and int(current_time.strftime("%H")) == 8
    ):
        today_at_8_datetime = current_time.replace(
            hour=8, minute=0, second=0, microsecond=0
        )
        tomorrow_at_8_datetime = today_at_8_datetime + timedelta(days=1)
        slots_list = [
            tomorrow_at_8_datetime + timedelta(hours=work_hours + 1)
        ...
        for slot in slots_list:
            table.put_item(
                Item={
                    ...
                    "username": None,
                    "full_name": None,
                    "appointment_times": slot.strftime("%Y-%m-%d %H:%M")
                    + "-"
                    + (slot + timedelta(hours=1)).strftime("%H:%M"),
                }
            )
    if (
        current_time.strftime("%A") == "Thursday"
        ...
        slots_list = [
            next_sunday_at_8_datetime + timedelta(hours=work_hours + 1)
            for work_hours in range(9)
        ]
        for slot in slots_list:
            table.put_item(
    ...
    if (
        current_time.strftime("%A") != "Sunday"
        and int(current_time.strftime("%H")) == 17
    ):
        two_days_ago_time_str = (current_time - timedelta(days=2)).strftime(
            "%Y-%m-%d %H:%M"
        )
    ...
    if (
        ...
        four_days_ago_time_str = (current_time - timedelta(days=4)).strftime(
            "%Y-%m-%d %H:%M"
        )
...
def handle_telegram_event(event):
    ...
    if "message" in body:
        ...
        item = table.get_item(Key={"id": str(chat_id)})
        item = item.get("Item")
        ...
        if item and item["canceling_options_message_id"]:
            ...
            # check if we allow canceling (if there is more than 1 hour until appointment)
                ...
                datetime.strptime(appointment_id, "%Y-%m-%d %H:%M").replace(
                    tzinfo=israel_tz
                )
        ...
        if appointment_id:
            item = table.get_item(Key={"id": appointment_id})
            item = item.get("Item")
            appointment_times = item.get("appointment_times")
        ...
        if query_data == "keep":
            ...
            if (
                ...
                table.delete_item(Key={"id": str(chat_id)})
            else:
    ...
    if item:
        table.update_item(
            ...
            ExpressionAttributeValues={
                ":recent_use_timestamp": datetime.now(israel_tz).strftime(
                    "%Y-%m-%d %H:%M"
                )
            },
        )
...
def fetch_available_slots():
    response = table.scan(FilterExpression=Attr("is_available").eq(True))
...
def send_available_slots(chat_id, username, full_name):
    ...
    if available_slots:
        keyboard = [
            [{"text": slot["appointment_times"], "callback_data": slot["id"]}]
...
def ask_to_cancel_appointment(chat_id, appointment_id):
    ...
    table.update_item(
        ...
        ExpressionAttributeValues={
            ":canceling_options_message_id": str(sent_message_id)
        },
    )
...
def schedule_appointment(chat_id, appointment_id, message_id):
    # get the chosen slot
    item = table.get_item(Key={"id": str(appointment_id)})
    item = item.get("Item")
    if item.get(
        "is_available"
    ):  # also captures the check of expired adtes, asin in that case it is None
        appointment_times = item.get("appointment_times")
        ...
        chat_item = table.get_item(Key={"id": str(chat_id)})
...
def show_data_to_admin(chat_id):
    ...
    if scheduled_slots:
        sorted_scheduled_slots = sorted(
            scheduled_slots, key=lambda x: x["appointment_times"]
        )
